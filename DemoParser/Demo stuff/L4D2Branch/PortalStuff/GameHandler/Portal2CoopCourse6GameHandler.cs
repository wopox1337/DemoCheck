...
namespace DemoParser.Demo_stuff.L4D2Branch.PortalStuff.GameHandler
{
    internal class Portal2CoopCourse6GameHandler : OrangeBoxGameHandler
    {
        ...
        protected override PacketResult ProcessPacket(BinaryReader br)
        {
            ...
            stringBuilder.Append(string.Concat(currentTick));
            if (_startAdjustType == null && packetResult.CurrentPosition.Equals(new Result.Point3D(-9896f, -4400f, 3048f)))
            {
                _startAdjustType = "Portal2 co-op Atlas Gain Control";
                _startTick = CurrentTick;
            }
            else if (packetResult.CurrentPosition.Equals(new Result.Point3D(-11168f, -4384f, 3040.03125f)))
            {
